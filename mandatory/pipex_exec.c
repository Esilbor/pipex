...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/07/17 11:14:25 by bbresil           #+#    #+#             */
/*   Updated: 2023/08/24 14:33:28 by bbresil          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	ft_valid_directory(char **dir_paths, char *cmd, char **path, t_env *env)
{
	while (*dir_paths)
	{
		*path = ft_build_path(*dir_paths, cmd, env);
		if (!*path)
			ft_flush_exit(env, "pipex: permission denied:");
...
void	ft_exec_slash_cmd(t_env *env, char **cmd_opt)
{
	...
	else if (!access(cmd_opt[0], F_OK | X_OK | R_OK))
	{
...
char	**ft_exec(t_env *env, char *cmd)
{
	...
	if (*cmd && ft_arg_not_empty(cmd))
	{
		...
		ft_flush_exit(env, "pipex: empty command");
	return (cmd_opt);
}
